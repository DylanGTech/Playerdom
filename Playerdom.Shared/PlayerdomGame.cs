...
public class PlayerdomGame : Game
{
    ...
    protected override void Initialize()
    {
        ...
        Window.TextInput += (sender, e) =>
        {
            if (focusedObjectId.HasValue)
            {
                ...
                if (isTyping) typedMessage += e.Character;
            }
            else
            {
                KeyboardState ks = Keyboard.GetState();
    ...
    protected override void Draw(GameTime gameTime)
    {
        ...
        else
        {
            ...
            (double, double) copyP;
            lock (currentState.Chunks)
            {
                lock (currentState.Objects)
                {
                    lock (currentState)
                    {
                ...
                while (game._tcpClient.Connected)
                {
                    ...
                    if (obj != null)
                    {
                        ...
                        if (obj.Chunks != null && obj.GameObjects != null)
                        {
                            ...
                            lock (game.currentState.Chunks)
                            {
                                lock (game.currentState) 
                                {
                                    lock (game.currentState.Objects)
                                    {
                                        lock (game.currentState.Entities)
                                        {
